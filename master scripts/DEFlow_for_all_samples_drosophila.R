...
if (heatmaps == TRUE){
  ...
  if (analyze_all_samples == TRUE){
    ...
    topcpm <- t(scale(t(topcpm)))
    #df(file = "Top 5000 expressed genes.pdf", width = 12, height = 17, family = "Helvetica")
    png("Top 5000 expressed genes.png")
    heatmap.2(topcpm, col = col.pan, Rowv=TRUE, scale="column",
              trace="none", dendrogram="column", cexRow=1, cexCol=1.4, density.info="none",
              margin=c(10,9), lhei=c(2,10), lwid=c(2,6), main = "5000 Highest expressed genes",
              labRow = FALSE)
    dev.off()
  }
